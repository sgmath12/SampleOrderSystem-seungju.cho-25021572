...
반도체 샘플(시료) 생산 주문을 등록·승인·생산·출고까지 관리하는 콘솔 애플리케이션. 요구사항과 개발 방법론은 [`CLAUDE.md`](./CLAUDE.md), 도메인 규칙과 시나리오는 [`PRD.md`](./PRD.md) 참고.
...
```
[1] 시료 관리      — 시료 등록 / 조회(재고 포함) / 이름 검색
[2] 시료 주문      — 시료ID/고객명/수량 입력 → RESERVED 주문 생성
[3] 주문 승인/거절 — RESERVED 목록 확인 후 승인(재고충분→CONFIRMED / 부족→PRODUCING) 또는 거절(REJECTED)
[4] 모니터링      — 상태별 주문 수 집계, 시료별 재고 현황(여유/부족/고갈)
[5] 생산라인      — 대기 중인 생산 작업(FIFO) 확인, 생산 완료 처리(PRODUCING→CONFIRMED)
[6] 출고 처리      — CONFIRMED 주문 → RELEASE 전환
[0] 종료
```
...
SampleOrderSystem/
  ...
  CLAUDE.md               요구사항 스펙 + 개발 방법론
  PRD.md                  제품 요구사항 문서 (도메인 규칙, 수율 계산 예시 등)
```
